...
// // ServeSQLServer starts accepting SQL Server connections from the provided listener.
// func (s *ProxyServer) ServeSQLServer(listener net.Listener) error {
// 	s.log.Debug("Started SQL Server proxy.")
// 	defer s.log.Debug("Database SQL Server exited.")
// 	for {
// 		clientConn, err := listener.Accept()
// 		if err != nil {
// 			if utils.IsOKNetworkError(err) || trace.IsConnectionProblem(err) {
...
// 		go func() {
// 			defer clientConn.Close()
// 			err := s.SQLServerProxy().HandleConnection(s.closeCtx, clientConn)
// 			if err != nil {
// 				s.log.WithError(err).Warn("Failed to handle SQL Server client connection.")
// 			}
// 		}()
// 	}
// }
...
func (s *ProxyServer) handleConnection(conn net.Conn) error {
	...
	proxyCtx, err := s.Authorize(s.closeCtx, tlsConn, "", "")
	if err != nil {
		return trace.Wrap(err)
	}
...
func (s *ProxyServer) Authorize(ctx context.Context, tlsConn *tls.Conn, user, database string) (*common.ProxyContext, error) {
